...
fn main() {
    ...
    println!("    Entity 2 parent: {}", ecs_get_entity_parent(world, entity2));
    ...
    for i in 0..5 {
        ...
        println!("    Frame {}: time={:.3}s, delta={:.3}s", frame, time, delta);
    }
    println!();
    ...
    let has_transform = ecs_get_transform_component(
        world, entity1,
        pos.as_mut_ptr(), rot.as_mut_ptr(), scale.as_mut_ptr(),
    );
    if has_transform {
        println!("    Position: ({}, {}, {})", pos[0], pos[1], pos[2]);
        println!("    Rotation: ({}, {}, {}, {})", rot[0], rot[1], rot[2], rot[3]);
        println!("    Scale: ({}, {}, {})\n", scale[0], scale[1], scale[2]);
    }
    
    println!("[11] 销毁实体...");
    ecs_destroy_entity(world, entity2);
    println!("    Entity count after destroy: {}\n", ecs_entity_count(world));
    ...
    scripting_register_sync_callback(
        ...
        test_callback, cb_desc.as_ptr(), cb_sig.as_ptr(),
        5,
    );
