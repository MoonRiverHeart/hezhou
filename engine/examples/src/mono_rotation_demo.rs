...
fn main() {
    ...
    loop {
        ...
        let dt = 0.016f32;
        let _increment = executor
            .call("UpdateRotation", ScriptValue::from_float(dt))
            .expect("Call failed");
        ...
        if frame_count % 60 == 0 {
            ...
            if current_speed != last_speed {
                println!(
                    "\n[HotReload] Speed changed: {}°/s -> {}°/s",
                    last_speed, current_speed
                );
                last_speed = current_speed;
            }

            let angle_result = executor
                .call("GetCurrentAngle", ScriptValue::from_int(0))
                .expect("Failed");
            let angle = angle_result.float_value;

            println!(
                "    Frame {}: angle = {:.1}°, speed = {}°/s",
                frame_count, angle, current_speed
            );
        }
...
fn recompile_mono_dll() {
    ...
    if !output.status.success() {
        ...
        println!(
            "[HotReload] {}",
            stdout.lines().last().unwrap_or("编译成功")
        );
    }
}
