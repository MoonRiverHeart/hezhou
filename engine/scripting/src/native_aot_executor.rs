...
impl ScriptExecutor for NativeAotExecutor {
    fn load(&mut self, path: &str) -> Result<(), ScriptError> {
        unsafe {
            ...
            let get_rotation_speed: GetRotationSpeedFn =
                *library.get(b"csharp_get_rotation_speed").map_err(|e| {
                    ScriptError::SymbolNotFound(format!("csharp_get_rotation_speed: {}", e))
                })?;

            let set_rotation_speed: SetRotationSpeedFn =
                *library.get(b"csharp_set_rotation_speed").map_err(|e| {
                    ScriptError::SymbolNotFound(format!("csharp_set_rotation_speed: {}", e))
                })?;

            let reset_rotation: ResetRotationFn =
                *library.get(b"csharp_reset_rotation").map_err(|e| {
                    ScriptError::SymbolNotFound(format!("csharp_reset_rotation: {}", e))
                })?;
    ...
    fn reload(&mut self) -> Result<(), ScriptError> {
        Err(ScriptError::UnsupportedOperation(
            "NativeAOT does not support hot reload. Use Mono for development.".to_string(),
        ))
    }
    ...
    fn call(&self, method: &str, args: ScriptValue) -> Result<ScriptValue, ScriptError> {
        match method {
            "GetRotationSpeed" => {
                let speed = self.get_rotation_speed.ok_or(ScriptError::NotInitialized)?();
            ...
            "SetRotationSpeed" => {
                ...
                self.set_rotation_speed.ok_or(ScriptError::NotInitialized)?(speed);
                Ok(ScriptValue::null())
            }
    ...
    fn get_rotation_speed(&self) -> Result<f32, ScriptError> {
        self.get_rotation_speed.ok_or(ScriptError::NotInitialized)?().pipe(|speed| Ok(speed))
    }
