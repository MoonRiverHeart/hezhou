use hezhou_scripting::{CallbackDescriptor, CallbackRegistry, ScriptValue};
...
fn test_registry_different_contexts() {
    ...
    assert_eq!(
        registry.trigger_sync("double", arg).unwrap().get_int(),
        Some(20)
    );
    assert_eq!(
        registry.trigger_sync("triple", arg).unwrap().get_int(),
        Some(30)
    );
}
...
fn test_registry_error_handling() {
    ...
    let result = registry
        .trigger_sync("error_test", ScriptValue::null())
        .unwrap();
...
fn test_registry_unregister() {
    ...
    extern "C" fn identity(arg: ScriptValue, _ctx: usize) -> ScriptValue {
        arg
    }
    ...
    assert!(registry
        .trigger_sync("temp", ScriptValue::from_int(1))
        .is_ok());
...
fn test_registry_not_found() {
    ...
    assert!(registry
        .trigger_sync("nonexistent", ScriptValue::null())
        .is_err());
}
...
fn test_registry_list_callbacks() {
    ...
    extern "C" fn sync_cb(_arg: ScriptValue, _ctx: usize) -> ScriptValue {
        ScriptValue::null()
    }
    ...
    registry.register_sync(
        "s1".to_string(),
        sync_cb,
        CallbackDescriptor::new_sync("s1", "", ""),
        0,
    );
    registry.register_async(
        "a1".to_string(),
        async_cb,
        CallbackDescriptor::new_async("a1", ""),
        0,
    );
    registry.register_task(
        "t1".to_string(),
        task_cb,
        CallbackDescriptor::new_task("t1", "", true),
        true,
        0,
    );
