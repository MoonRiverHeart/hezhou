...
pub struct MonoRotationRenderer {
    glfw: Glfw,
    window: PWindow,
...
impl MonoRotationRenderer {
    pub fn new(width: u32, height: u32, title: &str) -> Result<Self, String> {
        unsafe {
            ...
            let engine_name = CString::new("Hezhou Mono").unwrap();
            ...
            println!("[Mono] Loading C# script...");
            let current_dir = std::env::current_dir()
                .map_err(|e| format!("Failed to get current dir: {}", e))?;
            
            let scripts_dir = if current_dir.ends_with("examples") {
                current_dir.parent().unwrap().join("scripts")
            } else {
                current_dir.join("scripts")
            };
            
            let dll_path = scripts_dir
                .join("bin/Mono/Release/net8.0/RotationScript.Mono.dll")
                .to_str()
                .unwrap()
                .to_string();
            
            println!("[Mono] DLL path: {}", dll_path);
            
            let executor = MonoExecutor::new("HezhouScripts", "RotationController")
                .map_err(|e| format!("Failed to create MonoExecutor: {:?}", e))?;
            
            let mut executor = executor;
            executor.load(&dll_path)
                .map_err(|e| format!("Failed to load DLL: {:?}", e))?;
            
            let speed = executor.get_rotation_speed()
                .map_err(|e| format!("Failed to get speed: {:?}", e))?;
            println!("[Mono] Initial rotation speed: {}°/s", speed);
    ...
    fn recompile_mono_dll(&self) -> Result<String, String> {
        let build_script = self.scripts_dir.join("build_mono.ps1");
        ...
        let assembly_name = stdout.lines()
            .find(|line| line.starts_with("AssemblyName:"))
            .map(|line| line.strip_prefix("AssemblyName:").unwrap().trim())
            .ok_or("Failed to parse assembly name from output")?;
        
        let dll_path = self.scripts_dir
    ...
    pub fn draw_frame(&mut self, current_angle: &mut f32) -> Result<bool, String> {
        unsafe {
            ...
            if r_key_state == Action::Press && !self.r_key_pressed {
                ...
                match self.recompile_mono_dll() {
                    Ok(new_dll_path) => {
                        println!("[HotReload] Compile success! New DLL: {}", new_dll_path);
                        self.dll_path = new_dll_path.clone();
                        
                        match self.executor.load(&new_dll_path) {
                            Ok(_) => {
                                println!("[HotReload] Assembly loaded, calling ResetAll...");
                                
                                self.executor.call("ResetAll", ScriptValue::from_int(0))
                                    .map_err(|e| format!("Failed to call ResetAll: {:?}", e))?;
                                
                                let speed = self.executor.get_rotation_speed()
                                    .map_err(|e| format!("Failed to get speed: {:?}", e))?;
                                println!("[HotReload] Speed after reload: {}°/s\n", speed);
                            }
                            Err(e) => println!("[HotReload] Load error: {:?}", e),
                        }
                    }
                    Err(e) => println!("[HotReload] Compile error: {}", e),
                }
            ...
            let angle_increment = self.executor.call("UpdateRotation", ScriptValue::from_float(delta_time as f32))
                .map_err(|e| format!("Failed to call UpdateRotation: {:?}", e))?;
            
            *current_angle += angle_increment.float_value;
            ...
            if self.frame_count % 60 == 0 {
                let speed = self.executor.get_rotation_speed()
                    .map_err(|e| format!("Failed to get speed: {:?}", e))?;
                println!("Frame {}: angle={:.1}°, speed={}°/s", self.frame_count, *current_angle, speed);
            }
...
impl Drop for MonoRotationRenderer {
    fn drop(&mut self) {
        unsafe {
