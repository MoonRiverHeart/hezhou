...
impl TraceAnalyzer {
    ...
    pub fn begin_point(&mut self, name: &str, _category: &str) {
        ...
        self.active_points.lock().insert(name.to_string(), start_time);
    }
    ...
    pub fn export_json(&self) -> String {
        ...
        for (i, point) in points.iter().enumerate() {
            ...
            let cat = unsafe { std::ffi::CStr::from_ptr(point.category).to_str().unwrap_or("") };
        ...
        for (_, counter_values) in counters.iter() {
            for counter in counter_values.iter() {
                ...
                let name = unsafe { std::ffi::CStr::from_ptr(counter.name).to_str().unwrap_or("") };
                let cat = unsafe { std::ffi::CStr::from_ptr(counter.category).to_str().unwrap_or("") };
