...
impl CrashHandler {
    ...
    fn setup_signal_handlers(&mut self) {
        use libc::{SIGSEGV, SIGBUS, SIGFPE, SIGILL, SIGABRT};
    ...
    fn write_crash_file(report: &CrashReport) {
        ...
        let message = unsafe { std::ffi::CStr::from_ptr(report.message).to_str().unwrap_or("") };
        ...
        if let Ok(mut file) = std::fs::File::create(&crash_file) {
            ...
            let frames = unsafe {
                std::slice::from_raw_parts(report.frames, report.frame_count as usize)
            };
            
            for (i, frame) in frames.iter().enumerate() {
                let symbol = unsafe { std::ffi::CStr::from_ptr(frame.symbol).to_str().unwrap_or("") };
                let file_name = unsafe { std::ffi::CStr::from_ptr(frame.file).to_str().unwrap_or("") };
                let line = format!(
                    "#{} {} @ {}:{} (0x{:x})\n",
