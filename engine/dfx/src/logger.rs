...
impl Logger {
    ...
    fn write_console(&self, entry: &LogEntry) {
        ...
        let module = unsafe { std::ffi::CStr::from_ptr(entry.module).to_str().unwrap_or("") };
        let message = unsafe { std::ffi::CStr::from_ptr(entry.message).to_str().unwrap_or("") };
        
        println!("[{}][{}][{}] {}", time, entry.level.as_str(), module, message);
    }
    ...
    fn write_file(&self, file: &std::fs::File, entry: &LogEntry) {
        ...
        let line = format!(
            "[{}][{}][T:{}][{}][{}:{}] {}\n",
            time, entry.level.as_str(), entry.thread_id, module, file_name, entry.line, message
        );
...
impl LogManager {
    ...
    pub fn create_logger(&mut self, name: &str) {
        ...
        self.loggers.lock().insert(name.to_string(), Box::new(logger));
    }
