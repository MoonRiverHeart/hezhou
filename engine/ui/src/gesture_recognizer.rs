            ...
            for gesture in &mut self.active_gestures {
                if gesture.target == event.target {
                    ...
                    if distance > 10.0 && gesture.gesture_type == GestureType::Tap {
                        ...
                        return Some(Gesture::new(GestureType::Pan, gesture.target)
                            .with_data(GestureData::Pan(PanData {
                    ...
                    match gesture.gesture_type {
                        GestureType::Tap => {
                            if elapsed < 500 && distance < 10.0 {
                                ...
                                println!("[GestureRecognizer] {} recognized: target={}", 
                                    match gesture_type {
                                        GestureType::Tap => "Tap",
                                ...
                                return Some(Gesture::new(gesture_type, gesture.target)
                                    .with_data(GestureData::Tap(TapData::new(touch.x, touch.y)
