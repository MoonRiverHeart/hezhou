...
fn main() {
    ...
    let panel = Box::new(Panel::new()
        .with_layout_type(LayoutType::Absolute));
    
    tree.lock().set_root(panel);
    
    let panel_id = tree.lock().root.unwrap();
    
    let button1 = Box::new(Button::new("Click Me")
    ...
    if let Some(btn) = tree.lock().get_widget_mut(button1_id) {
        btn.as_mut().set_layout(Layout::new(20.0, 20.0, 120.0, 40.0));
    }
    ...
    if let Some(lbl) = tree.lock().get_widget_mut(label_id) {
        lbl.as_mut().set_layout(Layout::new(20.0, 80.0, 200.0, 30.0));
    }
    ...
    ui_system.get_event_dispatcher().lock().dispatch_event(&mut touch_event);
    
    let render_data = tree.lock().generate_render_data();
    println!("\nRender data generated: {} draw commands", render_data.len());
    
    for (i, data) in render_data.iter().enumerate() {
        println!("  Widget {}: {} commands at ({}, {})", 
            i, data.draw_commands.len(), data.bounds.x, data.bounds.y);
    }
